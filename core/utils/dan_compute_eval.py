...
def eval_opt_kp(kps,comb, rec_eval_pts, gt_eval_pts):
    ...
    kp3d = kps #hip_1st_to_alpha(kps)

    # TODO: Rebuttal
    # Write out hard-coded indices for smpl and mirror evaluation seperately -> mpjpe
    # psnr on eval points
    # initial and final recon for tim
    # novel view synthesis for tim too.

    # smpl indices
    s_pelvis, s_lhip, s_rhip = 0,1,2
    s_lknee, s_r_knee, s_l_ankle = 4,5,7
    s_r_ankle, s_neck, s_l_sh = 8,12,16
    s_l_elb, s_l_wrist, s_r_sh = 18,20,17
    s_r_elb, s_r_wrist = 19, 21

    # mirror25 indices
    m_pelvis, m_lhip, m_rhip = 8,12,9
    m_lknee, m_r_knee, m_l_ankle = 13,10,14
    m_r_ankle, m_neck, m_l_sh = 11,1,5
    ...
    mirr_eval_joints = [m_pelvis, m_lhip, m_rhip, m_lknee, m_r_knee, m_l_ankle, m_r_ankle, m_neck, m_l_sh, 
    m_l_elb, m_l_wrist, m_r_sh, m_r_elb, m_r_wrist]
    ...
    filename = data_dir + f"/mirror_GT3D.pickle"
    from_pickle = load_pickle(filename)
    ...
    kp3d_final = kp3d_chosen[:, smpl_eval_joints]
    gt3d_chosen = gt3d_chosen[:, mirr_eval_joints]
    ...
    hip_index = 0 # based on rebuttal indices
